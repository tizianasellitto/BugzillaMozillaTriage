exports.main = function(options) {
    ...
    panel.port.on("getUserAssignedBugs", function(message) {
                  if(message!="")
                  var getUserAssignedBugs=[ { "assigned_to": [message] ,"include_fields":["id","status","summary","last_change_time"]} ];
                  else
                  var getUserAssignedBugs=[ { "assigned_to": ["tiziana.sel@gmail.com"],"include_fields":["id","status","summary","last_change_time" ]} ];
    ...
    panel.port.on("getUserQAcontactBugs", function(message) {
                  if(message!="")
                  var getUserQAcontactBugs=[ { "qa_contact": [message] ,"include_fields":["id","status","summary","last_change_time"]} ];
                  else
                  var getUserQAcontactBugs=[ { "qa_contact": ["tiziana.sel@gmail.com"],"include_fields":["id","status","summary","last_change_time" ]} ];
