...
function user(response) {
	if (response.result != null && response.result.users.length != 0) {
		document.getElementById("notification").innerHTML = "Get user Info of "
...
function userBugs(response) {
	if (response.result != null && response.result.bugs.length == 0) {
		...
		document.getElementById("notification").innerHTML = "Get Bugs reported by "
		...
		response.result.bugs.forEach(function(entry) {
			values += "<tr>";
			values += "<td> <a href='' onclick='javascript:openBug(" + entry.id
					+ ")'>" + entry.id + "</a> </td>";
...
function loadQueryResponse(response) {
	...
	document.getElementById("1").innerHTML = "Website bugs that are UNCO  <a href='' onclick='javascript:openUrl(\""
			+ url + "\")'>(" + response.result.bugs.length + ")</a>";
}
...
function loadQueryResponse2(response) {
	...
	var url = "https://bugzilla.mozilla.org/buglist.cgi?list_id=7119720&resolution=---&resolution=DUPLICATE&chfieldto=Now&chfield=%5BBug%20creation%5D&query_format=advanced&bug_status=UNCONFIRMED&chfieldfrom="
			+ dayString;
	document.getElementById("2").innerHTML = "Today's UNCO bugs for every product <a href='' onclick='javascript:openUrl(\""
			+ url + "\")'>(" + response.result.bugs.length + ")</a>";
}
...
function loadQueryResponse3(response) {
	...
	var url = "https://bugzilla.mozilla.org/buglist.cgi?list_id=7119507&resolution=---&resolution=DUPLICATE&chfieldto=Now&chfield=%5BBug%20creation%5D&query_format=advanced&bug_status=UNCONFIRMED&component=Untriaged&product=Firefox&chfieldfrom="
			+ dayString;
	document.getElementById("3").innerHTML = "Untriaged Firefox -1m <a href='' onclick='javascript:openUrl(\""
			+ url + "\")'>(" + response.result.bugs.length + ")</a>";
}
...
function getTriaging() {
	var listTab = document.getElementById("list");
	document.getElementById("triage").className = "attivo";
	document.getElementById("new").className = "";
	document.getElementById("list").className = "";
	...
	document.getElementById("resultsTriage").innerHTML = " - Loading Latest Unconfirmed Bugs";
	document.getElementById("notificationTriage").innerHTML = "";
...
function triagingLatestUnconfirmedBugs(response) {
	if (response.result != null && response.result.bugs.length == 0) {
		...
		var oldHTML = document.getElementById('resultsTriage').innerHTML;
		response.result.bugs.forEach(function(entry) {
			values += "<tr>";
			values += "<td> <a href='' onclick='javascript:openBug(" + entry.id
					+ ")'>" + entry.id + "</a> </td>";
		...
		values += "</table>";
		document.getElementById("resultsTriage").innerHTML = values;
	}
}